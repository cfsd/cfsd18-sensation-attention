...
class Attention {
 ...
 private:
  ...
  void ConeDetection();
  std::vector<std::vector<uint32_t>> NNSegmentation(Eigen::MatrixXd &pointCloudConeROI);
  std::vector<std::vector<uint32_t>> FindConesFromObjects(Eigen::MatrixXd &pointCloudConeROI, std::vector<std::vector<uint32_t>> &objectIndexList);
  Eigen::MatrixXd ExtractConeROI(Eigen::MatrixXd);
  ...
  double m_yBoundary = {};
  double m_zBoundary = {};
  double m_groundClusterDistance = {};
  double m_groundClusterSize = {};
