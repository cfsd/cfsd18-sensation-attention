...
Cone::Cone(double x, double y,double z,double saveDistance):
  m_x()
, m_y()
, m_z()
, m_hits()
...
bool Cone::shouldBeInFrame(){
  if(m_hits >= 3 && m_y > m_saveDistance && (m_missHit < 2 || (m_hits>12 && m_y<1)) && m_isValid){return true;}else{return false;}
}

bool Cone::shouldBeRemoved(){
  if((m_missHit > 2 && (m_hits<12 || m_y>1)) || m_y < m_saveDistance){return true;}else{return false;}
}
